...
function parseGeneToTraits(gene: string, method: string): void {
  ...
  for (
    let i: i32 = genesToArray.length - 1, j: i32 = 9 - 1;
    i >= 0;
    i -= geneDelimeter, j -= 1
  ) {
...
export function handleTokenMorphed(event: TokenMorphed): void {
  ...
  tokenMorphed.priceForGenomeChange = contract.priceForGenomeChange(
    tokenMorphed.tokenId
  );
  ...
  if (!oldGene.endsWith(newGeneString)) {
    if (oldGene != "0") {
      log.debug(
        "DEBUG INFO::  GENE has been changed OLD GENE is:  {} NEW GENE is: {}",
        [oldGene, newGeneString]
      );
